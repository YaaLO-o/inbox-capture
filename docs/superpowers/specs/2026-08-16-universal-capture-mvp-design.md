...
应用从 `%APPDATA%\obsidian\obsidian.json` 读取 Obsidian 仓库位置，优先使用当前打开且真实存在的仓库。每个本地自然日使用一个以日期命名的 Markdown 文件，同一天的内容持续追加，跨过午夜后自动写入新日期文件。如果没有找到可用仓库，界面会显示“未找到 Obsidian 仓库”，且不会改写其他位置。如果采集子文件夹或文件不存在，应用会自动创建。文件使用 UTF-8 编码，每次采集按照以下固定格式追加：
...
读取 Obsidian 配置并选择当前打开且真实存在的仓库，再根据采集发生的本地日期生成仓库内 `Universal Capture\YYYY-MM-DD.md` 的保存路径。配置缺失、格式无效或没有可用仓库时返回明确错误，不回退到仓库外的位置。
...
负责显示带可见拖动把手的悬浮按钮，在点击时发起采集请求，并根据返回结果短暂切换状态文字。把手通过受限 IPC 请求主进程移动当前窗口；界面不能直接访问 Node.js 或本地文件系统。
